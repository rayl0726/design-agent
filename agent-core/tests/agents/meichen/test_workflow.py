...
async def test_requirement_passed_through_workflow():
    ...
    with (
        patch("app.agents.meichen.skills.input_parser.parse_input", new_callable=AsyncMock) as mock_parse,
        patch("app.agents.meichen.skills.requirement_analyst.analyze_requirement", new_callable=AsyncMock) as mock_analyze,
        patch("app.agents.meichen.skills.concept_designer.design_concepts", new_callable=AsyncMock) as mock_concepts,
        patch("app.agents.meichen.skills.visual_designer.generate_visuals", new_callable=AsyncMock) as mock_visuals,
        patch("app.agents.meichen.skills.technical_designer.design_technical", new_callable=AsyncMock) as mock_technical,
    ):
